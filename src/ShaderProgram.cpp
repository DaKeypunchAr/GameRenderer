...
namespace DK
{
	...
	void Program::initialize(const std::string& vsFilePath, const std::string& fsFilePath)
	{
		...
		if (!linkProgram())
		{
			throw std::exception("Failed to link the program!");
		}
	...
	std::string Program::readFile(const std::string& filepath) const
	{
	...
	Program Program::colorRenderer = {};
	Program Program::colorCircleRenderer = {};
}